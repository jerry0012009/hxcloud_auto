...
研究未知 API 的正确方法：
1. **读 JS chunk 文件**提取接口路径和参数（被动研究，零风险）
2. **只用 GET 接口**查询数据结构（只读，不影响数据）
3. **绝不调用未知的 POST/PUT/DELETE 接口**去"看看会怎样"
4. 如果必须测试写入操作，**先问用户确认**，用户同意后在可接受的范围内操作

本次事故原因：探索阶段随意调用了未知接口，导致不可逆的主表状态变更。
教训：**研究的目的是搞清楚接口文档，不是在生产服务器上做实验。**
